    ...
    public void drawFrame(int w, int h, Graphics2D g2) {
        ...
        while (iterator.hasNext()) {
            ...
            while (saw.hasNext() && obj.show) {
                Ship saws = (Ship) saw.next();
                Rectangle location = obj.getLocation();
                Rectangle loc = saws.getLocation();
                if (obj.collision(saws)) {
                    if (obj.img.equals(sprites.get("wall"))) {
                        if (location.y < loc.y) {
                            saws.move(0, 10);
                        }
            ...
            while (iterator.hasNext()) {
                Ship exit = (Ship) iterator.next();
                    ...
                    if (exit.collision(p1)) {
                        p1.save();
                        p1.saves+=1;
                        p1.setLocation(new Point(-1,-1));
                    }
                    if(exit.collision(p2)){
                        p2.save();
                        p1.saves+=1;
                        p2.setLocation(new Point(-1,-1));
                    }
                    if(exit.collision(p3)){
                        p3.save();
                        p1.saves+=1;
                        p3.setLocation(new Point(-1,-1));
                    }       
                exit.draw(g2, this);
            }
